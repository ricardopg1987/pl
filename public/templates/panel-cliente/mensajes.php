<?php
/**
 * Plantilla para la pestaña de mensajes del panel de cliente
 * 
 * Ruta: /public/templates/panel-cliente/mensajes.php
 */
...
$destinatario_id = isset($_GET['especialista_id']) ? intval($_GET['especialista_id']) : 0;
...
if ($accion === 'ver' && $mensaje_id > 0) {
    ...
    $mensaje = $wpdb->get_row($wpdb->prepare(
        ...
        $mensaje_id, $cliente_id, $cliente_id
    ));
    ...
    if ($mensaje->destinatario_id == $cliente_id && !$mensaje->leido) {
        $wpdb->update(
    ...
    $interlocutor_id = $mensaje->remitente_id == $cliente_id ? $mensaje->destinatario_id : $mensaje->remitente_id;
    $interlocutor_nombre = $mensaje->remitente_id == $cliente_id ? $mensaje->destinatario_nombre : $mensaje->remitente_nombre;
    ?>
    ...
    <div class="sgep-mensaje-detail">
        <div class="sgep-mensaje-detail-header">
            ...
            <div class="sgep-mensaje-meta">
                <?php if ($mensaje->remitente_id == $cliente_id) : ?>
    ...
    if ($destinatario_id > 0) {
        ...
        if ($destinatario && in_array('sgep_especialista', $destinatario->roles)) {
    ...
    // Si no hay destinatario específico, obtener todos los especialistas con los que he tenido citas
    $especialistas = array();
    if ($destinatario_id <= 0) {
        // Obtener especialistas con los que he tenido citas
        $especialistas_citas = $wpdb->get_results($wpdb->prepare(
            "SELECT DISTINCT c.especialista_id, u.display_name 
            FROM {$wpdb->prefix}sgep_citas c
            LEFT JOIN {$wpdb->users} u ON c.especialista_id = u.ID
            WHERE c.cliente_id = %d
            ORDER BY u.display_name",
            $cliente_id
        ));
        
        foreach ($especialistas_citas as $esp) {
            $especialistas[$esp->especialista_id] = $esp->display_name;
        }
        
        // Si no tengo citas, obtener especialistas de mis matches
        if (empty($especialistas)) {
            $especialistas_matches = $wpdb->get_results($wpdb->prepare(
                "SELECT DISTINCT m.especialista_id, u.display_name 
                FROM {$wpdb->prefix}sgep_matches m
                LEFT JOIN {$wpdb->users} u ON m.especialista_id = u.ID
                WHERE m.cliente_id = %d
                ORDER BY m.puntaje DESC, u.display_name",
                $cliente_id
            ));
            
            foreach ($especialistas_matches as $esp) {
                $especialistas[$esp->especialista_id] = $esp->display_name;
            }
        }
    }
            ...
            <div class="sgep-form-field">
                ...
                <?php else : ?>
                    <select id="sgep_destinatario_id" name="sgep_destinatario_id" required>
                        ...
                        <?php foreach ($especialistas as $id => $nombre) : ?>
